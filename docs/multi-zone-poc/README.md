...
> 狀態：P0–P2 + P3 核心已在真機 `192.168.0.70`（Goke GK7205V200 / OHLinux 4.9.37）驗證通過（2026-07-21）。
...
- `mztone   <g722_file> <dst_ip> <dst_port> [loop:1|0] [dur_sec:0=inf]`

## 後續（見評估文件 §六）
P4 規模/codec、P5 config+device-web 對接、P6 開機自啟+維運。真實部署各區改 IGMP join 真實組播 group（取代 PoC 的單播 port），termapp 改聽專屬中繼 group（改 config 一個值、非改 binary）。
