...
export type ZonesProbe =
  ...
  | { status: 'unsupported' }                    // 確定的舊韌體：HTTP 403/404 路由不支援 / 解析成物件但無 zones / 空陣列
...
- `probeSipMulticastZones`：`GET /get/sip/multicast/zones`，沿用 `deviceApi` 既有 https-first/token/GBK 修復管線。**須區分「舊韌體」與「傳輸失敗」**（斷鏈防護，見 §六 S2）：
  ...
  - 成功但解析成物件卻無 `zones`／空陣列，或 **HTTP 403/404**（路由確定不支援＝舊韌體；真機實測工廠 https 韌體回 403、mzweb 回 404）→ `{status:'unsupported'}`。
  - **401/5xx（尤其 mzweb 於 mzrelay3 暫時不可用回的 503）／逾時／傳輸失敗 → `{status:'error'}`**（安全占位、不暴露單槽卡；`error` 時**重試一次（~800ms）**後仍失敗才定案）。⚠ 非 403/404 的 HTTP 錯誤（尤其 5xx）絕不判 'unsupported'，否則 capable 設備瞬時 503 會誤顯危險單槽卡→斷鏈。
...
//   HTTP 403/404（路由確定不支援＝舊韌體）→'unsupported'；401/5xx/逾時/無回應→'error'。
//   真機實測：新版工廠 https 韌體(.72)未知 GET 路由回 403、我方 mzweb 回 404；capable mzweb 對 zones 絕不回 403/404。
//   ⚠ 關鍵：5xx（尤其 mzweb 於 mzrelay3 暫時不可用時回的 503）**不得**判 'unsupported'，
...
- **`src/shared/multicastZones.test.ts`**（jest，主要防護）：
  ...
  - `classifyZonesProbe`：`{ok:true,zones:[...]}→'zones'`、`{ok:true,zones:[]}→'unsupported'`、`{ok:false,httpStatus:403/404}→'unsupported'`、`{ok:false,httpStatus:500/503}→'error'`、`{ok:false,httpStatus:401}→'error'`、`{ok:false}(無回應)→'error'`。**500→'error' 為斷鏈防護核心斷言（不得回退成 'unsupported'）**。
