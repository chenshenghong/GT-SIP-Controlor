...
| 檔案 | 動作 | 職責 |
|---|---|---|
| `src/shared/multicastZones.ts` | 新 | 純函式 + 常數：`MZ_COUNT=16`、`MZ_CODEC_PAIRS`、`normalizeZones`、`validateZones`、`serializeZones`。無 Vue/DOM 相依，可 jest 單測 |
| `src/shared/types.ts` | 改 | 加 `MulticastZone` 介面 |
| `src/renderer/composables/deviceApi.ts` | 改 | 加 `getSipMulticastZones`、`setSipMulticastZones` |
| `src/renderer/composables/useMulticastZonesCapability.ts` | 新 | 掛載時探測 zones 能力，回三態 `capable` |
...
export interface MulticastZone {
  zone_id: number            // 1..16
  multicast_address: string  // 啟用/touched 時須 224.x.x.x – 239.x.x.x
...
```ts
// deviceApi.ts
// 成功回設備原始 zones 陣列（非空，可能少於 16 筆）；空陣列/失敗/不支援回 null。
// （空陣列＝設備未回報監聽區，對齊 device-web renderMulticastZones 對 !raw.length 的處理）
export async function getSipMulticastZones(ip: string): Promise<MulticastZone[] | null>
...
- `getSipMulticastZones`：`GET /get/sip/multicast/zones`。回應為物件且含**非空** `zones` 陣列 → 回該陣列；空陣列或其餘（業務錯誤、非預期形狀、transport 失敗、RESP_PARSE）→ 回 `null`。沿用 `deviceApi` 既有 https-first/token/GBK 修復管線。
- `setSipMulticastZones`：`POST /set/sip/multicast/zones`，body `{ zones }`。伺服器恆 HTTP 200，成功 `status:"success"`；驗證失敗回 `status:"error"` + `error_code:"E001"` + `details`（指名 zone_id）。函式解析出 `errorZoneId`/`message` 供 UI `alert`。

## 六、能力偵測與三態呈現

`useMulticastZonesCapability(ip)` 於 `DeviceDetail` 掛載時探測一次，回 `capable: Ref<'unknown' | true | false>`：

| 狀態 | 觸發 | SIP 頁組播區 | 「組播監聽區」分頁 |
|---|---|---|---|
| `unknown` | 探測進行中 | 顯示「偵測組播能力中…」占位 | 不顯示 |
| `true` | `getSipMulticastZones` 回陣列 | **完全隱藏**單槽卡 | 顯示，載入 16 區表 |
| `false` | 回 `null`（含 timeout/舊韌體） | 維持單槽卡 | 不顯示 |

- **保守判定**：一次性探測；失敗（`null`）保守當「不支援」（單槽卡照舊、不誤隱藏功能）。重開設備會重探。`.70` 實測 `GET /get/sip/multicast/zones` ~0.5s 穩定，transient 失敗罕見；此權衡明確記載，不做重試迴圈避免過度設計。
- 單槽卡與分頁的可見性皆由同一 `capable` 值驅動，避免不一致。

## 七、16 區表行為（對齊 device-web `renderMulticastZones`）

1. **載入**：`getSipMulticastZones` → `normalizeZones(raw)` 補滿 16 筆——`zone_id` 缺漏補停用佔位列 `{zone_id:i, multicast_address:"", multicast_port:0, priority:0, enabled:false, audio_codec:""}`，避免半張表無法新增。
2. **每區欄位**：組播位址(224–239)、埠(1024–65535)、優先權(1–16，越小越優先)、音頻編碼(`MZ_CODEC_PAIRS`=`[["G.711U","G.711 µ-law"],["G.722","G.722"]]`)、啟用核取。Zone 1 標註「＝SIP / 組播頁單槽同一份設定」。codec 回空/未知時插入「（請選擇編碼）」哨兵選中，避免 `<select>` 靜默 fallback 覆蓋現場設定。
3. **即時提示**：啟用區之間 `priority` 重複者標紅 + 卡頭警示（不阻擋輸入，儲存時才擋）。
4. **儲存**：`validateZones(rows)` →
   - **佔位列**（全空且停用）：略過驗證。
   - **touched 列**（啟用 或 任一欄曾填值）：完整驗證——位址 224–239、埠 1024–65535、優先權 1–16 整數、codec 必選。
   - **全域優先權唯一**：僅計已啟用區。
...
- **`src/shared/multicastZones.test.ts`**（jest，主要防護）：
  - `normalizeZones`：少於 16 筆補滿、亂序/缺漏 zone_id 正確補位、null → 16 筆佔位。
  - `validateZones`：位址邊界(223/224/239/240)、埠邊界(1023/1024/65535/65536)、優先權邊界(0/1/16/17、非整數)、codec 必選、佔位列略過、touched（停用但有值）仍驗、啟用區優先權重複偵測、多重錯誤聚合。
  - `serializeZones`：預設值套用（空 codec→G.722、空 port/prio→0）。
- deviceApi 兩函式：以既有 deviceApi 測試模式覆蓋（若無 axios mock 環境，靠純模組單測 + 真機驗收）。
- 元件/composable：純模組已涵蓋核心邏輯；元件層靠手動 + 真機驗收（能力三態、單槽卡隱藏、E001 alert）。
...
- **能力誤判**：transient 失敗被當「不支援」→ 保守（單槽卡照舊，不破壞既有），重開設備重探。可接受。
- **Zone 1 雙入口**：SIP 單槽卡（隱藏於 capable）與 16 區表 Zone 1 為同一份；capable 時單槽卡隱藏，杜絕雙寫斷鏈。
- **契約漂移**：`getSipMulticastZones` 回應形狀假設 `{zones:[...]}`；以 device-web/mzweb 實測為準，非預期形狀一律當 `null`（不支援），不冒險解讀。
