<template>
  <div class="dayu-detail">
    ...
    <div v-if="errMsg" class="retry-row">
      <input v-model="user" placeholder="帳號" class="cred-input" />
      <input v-model="pass" type="password" placeholder="密碼" class="cred-input" />
      <button class="retry-btn" :disabled="loading" @click="readMedia">重試</button>
    </div>
    <div class="note-box">
...
async function readMedia() {
  ...
  errMsg.value = ''
  try {
    const r = await window.electronAPI.dayuGetMedia(props.device.ip, user.value, pass.value)
    if (r.ok) {
      media.value = r.value
    } else {
...
.retry-row { display: flex; gap: 8px; margin-bottom: 0.75rem; }
.cred-input { background: rgba(0,0,0,0.3); border: 1px solid rgba(78,222,163,0.2); color: #e0f2e9; padding: 8px 12px; border-radius: 6px; font-size: 0.9rem; }
.cred-input:focus { outline: none; border-color: #4edea3; }
.retry-btn { background: linear-gradient(135deg, #4edea3, #3bc991); border: none; color: #0c1324; padding: 8px 20px; border-radius: 8px; cursor: pointer; font-weight: 600; }
